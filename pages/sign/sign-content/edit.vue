    ...
    methods: {
      ...
      submit() {
        ...
        this.$refs.form.validate().then((res) => {
			let info = {...res}
			info.last_modify_date = new Date().getTime()
          return this.submitForm(info)
        }).catch(() => {
        }).finally(() => {
          uni.hideLoading()
        })
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("category_id,sign_content_sn,content,state,is_del,sort,add_date,last_modify_date").get().then((res) => {
