...
export default {
	...
	methods: {
		...
		submitForm(value) {
			// 使用 clientDB 提交数据
			db.collection(dbCollectionName).doc(this.formDataId)
				.update(value)
				.then(res => {
					uni.showToast({
						title: '修改成功'
		...
		getDetail(id) {
			...
			db.collection(dbCollectionName)
				.doc(id)
				.field('category_id,image_url,month_sell_count,total_sell_count,state,is_del,add_date,last_modify_date')
				.get()
				.then(res => {
